<script lang="ts">
  ...
  function handleModalKeydown(event: KeyboardEvent, closeFunction: () => void) {
  ...
  async function saveEdit() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        endpoints = endpoints.map(ep => (ep.id === editingEndpoint!.id ? updatedEndpoint : ep));
  ...
  async function deleteEndpoint() {
    ...
    isSubmitting = true;
    try {
      const response = await fetch(`/api/endpoints/${deletingEndpoint.id}`, {
        method: 'DELETE',
      });

      if (response.ok) {
        endpoints = endpoints.filter(ep => ep.id !== deletingEndpoint!.id);
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-indigo-50">
  ...
  >
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="text-center">
        ...
        <p class="mt-2 text-slate-600 font-medium">Build and manage your API endpoints</p>
      </div>
    </div>
  </header>
  ...
  {#if message}
    ...
    >
      ...
      >
        <div class="w-2 h-2 rounded-full {isError ? 'bg-red-500' : 'bg-emerald-500'}"></div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-5 gap-8">
      ...
      <div class="xl:col-span-2">
        <div class="sticky top-8">
          ...
          >
            <div class="bg-gradient-to-r from-violet-600 to-violet-800 px-6 py-4">
              <h2 class="text-xl font-semibold text-white">Create New Endpoint</h2>
            </div>

            <form on:submit|preventDefault={addEndpoint} class="p-6 space-y-6">
              <!-- Name Field -->
              <div class="space-y-2">
              ...
              <!-- Path Field -->
              <div class="space-y-2">
                <label for="path" class="block text-sm font-semibold text-slate-700">
                  API Path
                </label>
                <input
                  type="text"
      ...
      <div class="xl:col-span-3 space-y-6">
        ...
        {#if endpoints.length === 0}
          ...
          >
            ...
            <h3 class="text-xl font-semibold text-slate-700 mb-2">No endpoints yet</h3>
            <p class="text-slate-500">Create your first API endpoint using the form</p>
          </div>
        {:else}
          <div class="space-y-4">
                ...
                <div class="p-6">
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <button
                        on:click={() => confirmDelete(endpoint)}
    ...
    >
      ...
      >
        <div class="bg-gradient-to-r from-purple-600 to-indigo-600 px-6 py-4">
          <h3 id="edit-modal-title" class="text-xl font-semibold text-white">Edit Endpoint</h3>
          <p id="edit-modal-description" class="sr-only">
            Use this form to edit the API endpoint details. Press Escape to close.
          </p>
        </div>

        <form on:submit|preventDefault={saveEdit} class="p-6 space-y-4">
          <div class="space-y-2">
            <label for="edit-name" class="block text-sm font-semibold text-slate-700">
              Endpoint Name
            </label>
            <input
              type="text"
          ...
          <div class="space-y-2">
            <label for="edit-path" class="block text-sm font-semibold text-slate-700">
              API Path
            </label>
            <input
              type="text"
          ...
          <div class="space-y-2">
            <label for="edit-method" class="block text-sm font-semibold text-slate-700">
              HTTP Method
            </label>
            <select
              id="edit-method"
          ...
          <div class="space-y-2">
            <label for="edit-description" class="block text-sm font-semibold text-slate-700">
              Description
            </label>
            <textarea
              id="edit-description"
        ...
        <div class="p-6">
          <div class="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p id="delete-modal-description" class="text-slate-600">
                This action cannot be undone. Press Escape to close.
              </p>
            </div>
          </div>
          ...
          <div class="bg-slate-50 rounded-lg p-3 mb-6">
            <p class="text-sm text-slate-600">
              <span class="font-medium">{deletingEndpoint.name}</span><br />
